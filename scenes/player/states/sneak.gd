...
func _on_enter(actor: Node, _blackboard: Blackboard):
	actor.animation_tree.set("parameters/Animation/conditions/sneak", true)
...
func _on_update(_delta: float, _actor: Node, _blackboard: Blackboard):
	...
	if _actor.velocity == Vector2.ZERO:
		...
		_actor.animation_tree.set("parameters/Animation/conditions/sneak", _actor.active_weapon_index == 0 or !_actor.aiming)
		_actor.animation_tree.set("parameters/Animation/conditions/sneak", _actor.active_weapon_index != 0 and _actor.aiming)
