...
const MAX_SLICE_THREADS := 2
var slice_threads: Array[Thread] = []
var active_tasks: Array[ChunkSliceTask] = []
...
func _process_chunk_slicing():
	...
	for i in range(slice_threads.size() - 1, -1, -1):
		var thread = slice_threads[i]
		var task = active_tasks[i]

		if task.is_done:
			thread.wait_to_finish()
			_spawn_chunk_with_data(task.chunk_pos, task.result)
			slice_threads.remove_at(i)
			active_tasks.remove_at(i)

	# Launch new threads
	while slice_threads.size() < MAX_SLICE_THREADS and not slice_queue.is_empty():
		var task = slice_queue.pop_front()
		var thread := Thread.new()
		thread.start(Callable(self, "_threaded_slice_prefab").bind(task))

		slice_threads.append(thread)
